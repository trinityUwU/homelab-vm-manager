"""Réseau côté invité : détection du type de machine, et bascule IP statique
pour une VM QEMU pure (une VM QEMU n'a pas de net0 Proxmox — le durcissement
côté invité tient, contrairement à un LXC où Proxmox réinjecte sa config
réseau à chaque démarrage du conteneur, voir proxmox_host.py).
"""
import time
...
def detect_machine_type(session: SSHSession) -> str:
    """« qemu » ou « lxc », déduit de la notation d'interface. Un conteneur LXC
    est relié par une paire veth, visible en `eth0@ifNN` dans `ip -o link show` ;
    une VM QEMU n'a jamais ce suffixe (interface physique/virtio directe)."""
    _code, out, _err = session.run("ip -o link show")
    return "lxc" if "@if" in out else "qemu"
...
def harden_static_persistence(session: SSHSession, iface: str) -> None:
    ...
    script = (
        ...
        # est notre source de vérité, l'unit peut être active hors symlink standard).
...
def restart_networking(session: SSHSession, iface: str) -> None:
...
def write_resolv_conf(session: SSHSession, static_ip: str) -> None:
...
def apply_static_ip(session: SSHSession, static_ip: str) -> str:
    ...
    write_resolv_conf(session, static_ip)
    harden_static_persistence(session, iface)
    restart_networking(session, iface)
    return iface
